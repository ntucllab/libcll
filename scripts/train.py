...
def main(args):
    ...
    train_loader, valid_loader, test_loader, input_dim, num_classes, Q, class_priors = prepare_dataloader(
        args.dataset._name, 
...
def parse_args():
    ...
    parser.add_argument("--config_file", dest="training.config_file", type=str, default="libcll/configs/base.yaml")
    parser.add_argument("--model", dest="model._name", type=str, default="dense")
    parser.add_argument("--model_path", dest="training.model_path", type=str, default=None)
    parser.add_argument("--dataset", dest="dataset._name", type=str, default="mnist")
    parser.add_argument("--valid_type", dest="training.valid_type", type=str, default="URE")
    parser.add_argument("--num_cl", dest="dataset.num_cl", type=int, default=1)
    parser.add_argument("--valid_split", dest="training.valid_split", type=float, default=0.1)
    parser.add_argument("--eval_epoch", dest="training.eval_epoch", type=int, default=10)
    parser.add_argument("--output_dir", dest="training.output_dir", type=str, default=None)
    parser.add_argument("--batch_size", dest="training.batch_size", type=int, default=256)
    ...
    parser.add_argument("--transition_matrix", dest="dataset.transition_matrix", type=str, default="uniform")
    ...
    args = parse_config(parser, getattr(args, "training.config_file"), args=sys.argv[1:])
    return args
