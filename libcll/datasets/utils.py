...
def collate_fn_multi_label(data):
    ...
    y = []
    for (x_i, y_i) in data:
        x.append(x_i.unsqueeze(0).repeat(y_i.shape[0], *[1 for _ in range(len(x_i.shape))]))
        y.append(y_i)
...
def Weak(num_classes, seed=1126):
    if num_classes != 10:
        raise ValueError(f'Weak Distribution have only implement for num_classes=10 datasets.')
...
def Strong(num_classes, seed=1126):
    if num_classes != 10:
        raise ValueError(f'Strong Distribution have only implement for num_classes=10 datasets.')
    rng = np.random.default_rng(seed=seed)
    Q = torch.zeros((num_classes, num_classes))
...
def biased_on_one(num_classes, max_prob=0.9, seed=1126):
    rng = np.random.default_rng(seed=seed)
    Q = (torch.ones(num_classes, num_classes) - torch.eye(num_classes)) * (1 - max_prob) / (num_classes - 2)
    for i in range(num_classes):
...
def get_transition_matrix(transition_matrix, num_classes, noise=0.1, seed=1126):
    ...
    if transition_matrix not in Q_LIST:
        raise ValueError(f'The transition matrix {transition_matrix} didn\'t implemented.')
    args = {
